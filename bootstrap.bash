...
AAB_BOOTSTRAP_REF="${AAB_BOOTSTRAP_REF:-generated/add/modal-cli-uv-tool}"
...
# (ruff, pre-commit, Modal); the private autocuda package is installed separately
...
install_uv_tools() {
    ...
    while IFS= read -r line; do
        ...
        case "$line" in
            ...
            modal) line="modal==${MODAL_VERSION}" ;;
        esac
        case "$line" in
            *==?*) ;;
            *)
